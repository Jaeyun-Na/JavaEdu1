...
public class objectOriented_8_상속과다형성 {
	 ...
	 * 이런 경우 고객의 등급이 하나라도 추가되거나 삭제되면 유지보수가 매우 복잡해짐.
	 * 
	 ...
	 * 
	 * -상속을 항상 사용하는것이 좋을까
	 * 당연히 그렇지 않다. 'IS-A 관계(is a relationship: inheritance)'라는 용어가 있음.
	 * IS-A 관계한 일반적인 개념과 구체적인 개념의 관계임. 즉 '사람은 표유류이다'와 같은 관계. 상속은 IS-A 관계에서 사용하는것이 가장 효율적임. 일반 클래스를 점차 구체화 하는 상황에서 상속을 사용하는 것. 
	 * 상속을 사용하면 많은 장점이 있찌만, 하위 클래스가 상위 클래스형에 종속되기 때문에 이질적인 클래스 간에 상속을 사용하지 않는 것이 좀음. 다순히 코드를 재사용할 목적으로 서로 관련이 없는 개념의 클래스들을 상속 관계로 사용하는 것은 좋지 않은 코드 작성법임.
	 * 예를 들어 과목을 나타내는 Subject클래스가 있으며 과목 아이디, 이름을 멤버 변수로 가지고 get(),set() 메서드도 제공.
	 * (inheritance2 - Subject)
	 * 
	 * 이제 Student(학생) 클래스를 만들어야함. 모든 학생은 전공 과목(Subject)를 가지고 있음. 그러므로 Subject 클래스에서 제공하는 여러 메서드를 활용하면 좋을 것 같음.
	 * Student클래스가 Subject 클래스를 상속받으면 될까? 아님 왜냐하면 Subject가 Student를 포괄하는 개념의 클래스가 아니기 떄문. 또한 Student 클래스를 상속받는 다른 클래스가 있을 수 있음.
	 * 이런 경우에는 'HAS-A 관계(has a relationship; association))'로 표현함.
	 * HAS-A 관계란 한 클래스가 다른 클래스를 소유한 관계임. Subject는 Student에 포함되어 Student의 멤버 변수로 사용하는 것이 적절함.
	 * 
	 * class Student{
	 * 	Subject majorSubject;
	 * }
	 * 
	 * 상속을 코드 재사용 개념으로 이해하면 안되는 이유가 재사용할 수 있는 코드가 있다고 해서 무조건 상속을 받는 것은 아님. 상속을 사용하면 크,ㄹ래스 간의 결합도가 높아져서 상위 클래스의 변화가 하위 클래스에 미치는 영향아 쿰.
	 * 따라서 상속은 '일반적인 클래스'와 '구체적인(확장되는) 클래스'의 관계에서 구현하는 것이 맞다.
	 * 
	 * 	-여러 클래스를 한 번에 상속받을 수 있는가?
	 * 한 클래스가 여러 클래스를 상속받는 것을 다중 상속이라고함. 자바 이전의 객체 지향 언어인 C++는 다중 상속을 지원했지만, 자바는 다중 상속을 지원하지 않음. 여러 클래스에서 상속을 받으면 그만큼 다양한 기능을 상속받을 수 있는 장점이 있을 텐데 자바는 왜 다중 상속을
	 * 지원하지 않을 걸까? 그 이유는 다중 상속으로 인한 모호성 때문임. 예를 들어 두 개 이상의 상위 클래스에 같은 이름의 메서드가 정의되어 있다면, 다중 상속을 받는 하위 클래스는 어떤 메서드를 상속받을지 모호해짐. 객체 지향에서 다중 상속의 모호성에 대한 예가
	 * 다이아몬드 문제(diamond problem)임. C++ 같은 언어에서는 문접적으로 이러한 문제를 해결하지만, 자바에서는 다중 상속의 장점보다는 모호함을 없애느느 쪽을 선택해 다중 상속을 사용하지 않는 것임.
	 * 따라서 extends 예약어 뒤에 오는 클래스는 반드시 한 개여야 함.
	 * 
	 * 08-6 다운 캐스팅과 instanceof
	 * 
	 *  -하위 클래스로 형 변환, 다운 캐스팅
	 *  앞에서 상위 클래스로 형 변환이 묵시적으로 이루어지는 과정을 알아보았다. 이번에는 다시 하위 클래스로 형 변환이 되는 과정을 알아보자.
	 *  
	 * 						Animal 클래스
	 * 				 			|
	 * 			------------------------------------
	 ...
	 *		Human 클래스		tiger 클래스			Eagle 클래스
	 *
	 *	위 같은 계릉 구조에서 상위 클래스를 자료형으로 선언하는 Animal ani = new Human(); 코드를 쓸 수 있음. 이때 생성된 인스턴스 Human은 Animal형임. 이렇게 Animal형으로 형 변환이 이루어진 경우에는 Animal 클래스에서 선언한 메서드와 멤버 변수만
	 *	사용할 수 있음. 다시 말해 Human 클래스에 더 많은 메서드가 구현되어 있고 다양한 멤버 변수가 있다고 하더라도 자료형이 Animal형인 상태에서는 사용할 수가 없음. 따라서 필요에 따라 다시 원래 인스턴스의 자료형(여기서는 Human형)으로 되돌아가야 하는 경우가 있음.
	 *	이렇게 상위 클래스로 형 변환되었던 하위 클래스를 다시 원래 자료형으로 형 변환하는 것을 다운 캐스팅(down casting)이라고 함.
	 *
	 *	-instanceof
	 *	상속 관계를 생각해 보면 모든 인간은 동물이지만 모든 동물이 인간은 아님. 따라서 다운 캐스팅을 하기 전에 상위 클래스로 형 변환된 인스턴스의 원래 자료형을 확인해야 변환할 때 오류를 막을 수 있음.
	 *	이를 확인하는 여약어가 바로 instanceof임.
	 *	instanceof는 다음과 같이 사용할 수 있음.
	 *
	 ...
	 *	}
	 *
	 *	위 코드에서 사용한 참조 변수 hAnimal은 원래 Human형으로 생성되었는데, Animal형으로 형 변환되었음. instanceof 예약어는 왼쪽에 있는 변수의 원래 인스턴스형이 오른쪽 클래스 자료형인가를 확인함.
	 *	코드를 보면 hAnimal이 Animal형으로 되어 있지만, 원래는 Human형으로 생성된 인스턴스인지 확인하는것. instanceof의 반환 값이 true이면 다운 캐스팅을 하는데, 이때는 Human human = (Human)hAnimal; 문장과 같이 명시적으로 자료형을 써 주어야함.
	 *	상위 클래스로는 묵시적으로 형 변환이 되지만, 하위 클래스로 형변환을 할 때는 명시적으로 해야 하기 때문. 만약 instanceof로 인스턴스형을 확인하지 않으면 오류가 발생할 수 있음.
	 *	
	 *  아래와 같이 원래 재료형이 Human형이 아닌 경우
	 *  
	 *  Animal ani = new Tiger();
	 *  Human h = (Human)ani;
	 *  
	 *  위와 같이 코딩해도 컴파이 ㄹ오류는 나지 않음. Tiger 인스턴스는 Animal형으로 자동을 형 변환이 됨. 변수 h의 자료형 Human과 강제 형 변환되는 ani의(Human)형이 동일하므로 컴파일 오류는 나지 않는 것임. 그대신 이코드를 실행하면 싱행 오류가 발생함.
	 *  따라서 참조 변수의 원래 인스턴스형을 정확히 확인하고 다운 캐스팅을 해얗 안전하며 이때 instanceof를 사용함. 그러면 원래 인스턴스형으로 다운 캐스팅하는 예를 살펴보자.
	 *  Animal 클래스를 상속받은 여러 동물 클래스가 있음. Human 클래스, Tiger 클래스, Eagle 클래스에는 각각 다른 메서드도 추가로 구현해보자.
	 *  
	 *  
	 * 
	 */
